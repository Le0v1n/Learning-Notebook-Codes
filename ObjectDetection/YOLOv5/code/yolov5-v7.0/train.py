...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # 这里names = {0: "item"}成立的条件是，开启了single_cls，且数据集标签>1
    names = {0: "item"} if single_cls and len(data_dict["names"]) != 1 else data_dict["names"]  # class names
    is_coco = isinstance(val_path, str) and val_path.endswith("coco/val2017.txt")  # COCO dataset

    # Model
    check_suffix(weights, ".pt")  # 检查--weights传入的路径是否以.pt结尾
    pretrained = weights.endswith(".pt")  # 检查--weights传入的路径是否以.pt结尾
    if pretrained:  # 使用了预训练权重
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch.load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
        # 💡  123 or "abc" --> "123" --> cfg or ckpt["model"].yaml：优先使用cfg，即--cfg传入的参数
        # 💡  anchors=hyp.get("anchors")): 说明data/hyps中也可以有"anchors"参数（优先使用的）
        # 💡  这里优先使用传入的参数，而非Model的默认参数
    ...
    # Freeze，默认值为 freeze=[0] --> 不冻结，💡  for x in range(0) --> 什么也不做
