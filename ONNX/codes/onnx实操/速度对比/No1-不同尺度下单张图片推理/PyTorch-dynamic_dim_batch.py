...
parser.add_argument('--desc', type=str, default='[PyTorch]', help='The description of this test')
...
# 加载 ONNX 模型，创建推理会话
model = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)  # ImageNet 预训练权重
...
for test_idx in track(range(args.test_times), description=args.desc):
    ...
    for _image_idx, img_name in enumerate(images_list):
        ...
        _probs = probs[_image_idx].detach().numpy()
        _indices = indices[_image_idx].detach().numpy()
