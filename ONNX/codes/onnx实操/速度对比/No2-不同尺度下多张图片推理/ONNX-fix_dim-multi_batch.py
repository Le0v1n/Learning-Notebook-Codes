...
parser.add_argument('--input-shape', type=parse_list, default=[16, 3, 640, 640], help='The shape of input')
parser.add_argument('--test-times', type=int, default=50, help='The number of test')
parser.add_argument('--desc', type=str, default='[Fix_axis][No_simplified][Multi-batch]', help='The description of this test')
parser.add_argument('--warm-up', type=str, default='no', help='model warm-up')
parser.add_argument('--verbose', action='store_true', help='')
args = parser.parse_args()  # 解析命令行参数

_shape = "x".join(map(str, args.input_shape))
onnx_weights_path = f'ONNX/saves/model-fix_dims-{_shape}.onnx'
print(f"Input shape: {_shape}")
# ==============================================================================
# 加载 ONNX 模型，创建推理会话
...
with open("ONNX/saves/Speed_record-ResNet18.txt", 'a') as f:
    f.write(f"|{args.input_shape}|{np.average(cost_times):.4f}s|")